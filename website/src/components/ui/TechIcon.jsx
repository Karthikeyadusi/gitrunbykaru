...
export function TechIcon({ name, size = 16, className = '' }) {
  const brandColor = BRAND_COLORS[name] || 'currentColor';
  const iconProps = {
    width: size,
    height: size,
    viewBox: "0 0 24 24",
  ...
  switch (name) {
    ...
    case 'React':
      return (
        <svg {...iconProps} fill="none" stroke={brandColor} strokeWidth="1.5">
    ...
    case 'Express':
      return (
        <svg {...iconProps} fill="none" stroke={brandColor} strokeWidth="2">
    ...
    case 'Flask':
      return (
        <svg {...iconProps} fill="none" stroke={brandColor} strokeWidth="2" strokeLinecap="round">
