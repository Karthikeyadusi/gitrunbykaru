...
export async function cloneRepo(url, logger = {}) {
  const log = logger;
  const spinner = log.createSpinner ? log.createSpinner(`Cloning  ${url}`) : { start: () => {}, succeed: (msg) => log.success?.(msg), fail: (msg) => log.error?.(msg) };
